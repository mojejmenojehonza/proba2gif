...
def enhance_image(image, contrast_factor=CONTRAST_FACTOR, brightness=BRIGHTNESS_ADJUSTMENT):
    ...
    tint_color = np.array(TINT_COLORS[SELECTED_TINT])
...
def process_images():
    ...
    image_files = sorted(glob.glob(f"{INPUT_DIRECTORY}/*.png"))
    frames = []
    ...
    calibration_crop = enhance_image(calibration_crop)
    ...
    # Save as GIF
    imageio.mimsave(OUTPUT_GIF, frames, duration=100)
