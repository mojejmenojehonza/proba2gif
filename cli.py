...
def process_images(input_dir, output_gif, calibration_image_path, frame_duration, tint, brightness, contrast_factor, loop_count):
    """Process images in the input directory and save them as a GIF."""
    start_path = glob.glob(os.path.join(input_dir, "*-0.png"))[0].split("-0.png")[0]
    image_paths = [f"{start_path}.png"]
    image_paths.extend([f"{start_path}-{i}.png" for i in range(len(glob.glob(os.path.join(input_dir, "*.png"))) - 1)])
    ...
    calibration_crop = crop_image(calibration_image, calibration_center, OUTPUT_SIZE)
    ...
    for image_path in image_paths:
        ...
        translations = [
            (tx, ty, calculate_image_difference(calibration_crop, crop_image(image, [tx, ty, sun_center[2]], OUTPUT_SIZE)))
        ...
        cropped_image = crop_image(image, [best_tx, best_ty, sun_center[2]], OUTPUT_SIZE)
        if difference > 64000:
        ...
        rotations = [
            (angle, calculate_image_difference(calibration_crop, rotate_image(cropped_image, angle)))
            for angle in range(0, FULL_ROTATION, ROTATION_STEP)
        ]
...
def main():
    ...
    process_images(args.input_dir, args.output_file, args.calibration_image, args.frame_duration, args.tint, args.brightness, args.contrast_factor, args.loop_count)
